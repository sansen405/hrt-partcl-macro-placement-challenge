"""TierPlace (CUDA) — same analytical pipeline as ``tierplaceheavy.py``,
tuned for **NVIDIA GPU + float32** only.

Differences from ``tierplaceheavy.py``:
    * Requires a visible CUDA device (raises ``RuntimeError`` otherwise).
    * All torch compute uses ``torch.float32`` on GPU (no CPU ``float64`` path).
    * Enables TF32 matmul/cudnn, cudnn benchmark mode, and high matmul precision
      where supported for better throughput on modern GPUs.
...
Usage (GPU machine with CUDA PyTorch):
    uv run evaluate submissions/will_seed/tierplaceheavy_cuda.py
    uv run evaluate submissions/will_seed/tierplaceheavy_cuda.py --all
"""
...
# ---------------------------------------------------------------------------
# CUDA / float32 runtime (this submission targets GPU only)
# ---------------------------------------------------------------------------


def _configure_cuda_training_prefs() -> None:
    """Throughput-oriented PyTorch settings for fp32 on NVIDIA GPUs."""
...
def _require_cuda_device(cuda_device: int = 0) -> torch.device:
    if not torch.cuda.is_available():
        raise RuntimeError(
            "tierplaceheavy_cuda requires CUDA and a GPU-enabled PyTorch build; "
...
class AnalyticalPlacer:
    ...
    def __init__(
        ...
        _configure_cuda_training_prefs()
    ...
    def _place_base(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        dev = _require_cuda_device(self._cuda_device)
        dt = torch.float32
